...
int mqtt3_handle_publish(struct mosquitto_db *db, struct mosquitto *context)
{
	...
	_mosquitto_log_printf(NULL, MOSQ_LOG_DEBUG, "Received PUBLISH from %s (d%d, q%d, r%d, m%d, '%s', %s (%ld bytes))",context->id, dup, qos, retain, mid, topic, (char *) payload, (long)payloadlen);
	if(qos > 0){
